...
public class RealTimeDialogAdapter extends BaseAdapter {
    ...
    Context mContext;
    private int mtype;//区分是否是第一次进去  第一次 mType=0；非第一次进去 mType=1；

    public RealTimeDialogAdapter(Context mContext, List<RealTimeRankEntity> list, int mType) {
    ...
    @Override
    public int getCount() {
        return mtype == 0 ? 1 : list.size();
    }

    @Override
    public Object getItem(int position) {
        return mtype == 0 ? list.get(0) : list.get(position);
    }

    @Override
    public long getItemId(int position) {
        return  mtype == 0 ? 0 : position;
    }

    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
//        RealTimeRankEntity realTimeRankEntity = list.get(0);
        RealTimeRankEntity realTimeRankEntity = mtype == 0 ? list.get(0) : list.get(position);
        Holder holder;
        if (convertView == null) {
            holder = new Holder();
            convertView = mInflater.inflate(R.layout.real_time_item_dialog_layout, null);
